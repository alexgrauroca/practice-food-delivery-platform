...
const testDBPrefix = "staff_test_authentication_service"
...
func TestRepository_FindStaff(t *testing.T) {
	...
	tests := []staffRepositoryTestCase[staff.FindStaffParams, staff.Staff]{
		{
			name: "when there is not an active staff with the email and restaurant id, " +
				"then it should return a staff not found error",
			insertDocuments: func(t *testing.T, coll *mongo.Collection) {
				mongodb.InsertTestDocument(
					t, coll, staff.Staff{
						Email:        "test@example.com",
		...
		{
			name: "when there is an active staff with the email and restaurant, then it should return the staff",
			insertDocuments: func(t *testing.T, coll *mongo.Collection) {
				mongodb.InsertTestDocument(
					t, coll, staff.Staff{
						Email:        "test@example.com",
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				...
				repo := staff.NewRepository(logger, tdb.DB, clock.FixedClock{FixedTime: now})
				got, err := repo.FindStaff(context.Background(), tt.params)
...
func TestRepository_FindStaff_UnexpectedFailure(t *testing.T) {
	...
	_, err := repo.FindStaff(context.Background(), staff.FindStaffParams{})
	assert.Error(t, err, "Expected an error due to unexpected failure")
