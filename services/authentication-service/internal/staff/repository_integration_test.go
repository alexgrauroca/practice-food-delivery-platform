...
func TestRepository_FindByEmail(t *testing.T) {
	...
	tests := []staffRepositoryTestCase[string, staff.Staff]{
		{
			name: "when there is not an active staff with the email, then it should return a staff not found error",
			insertDocuments: func(t *testing.T, coll *mongo.Collection) {
				mongodb.InsertTestDocument(t, coll, staff.Staff{
					Email:     "test@example.com",
					Password:  "fakehashedpassword",
		...
		{
			name: "when there is an active staff with the email, then it should return the staff",
			insertDocuments: func(t *testing.T, coll *mongo.Collection) {
				mongodb.InsertTestDocument(t, coll, staff.Staff{
